...
public class OfferSord30_19 {
    /**
     * 一开始的思路： 认为 按照顺序/或者在minStack 中保存所有 入栈过的顺序 那么 这样子 就涉及到排序
     * 即使minStack 使用链表 那么 当新元素入栈时 查找其在有序minStack中的位置 也许 O（n） 题目要求 O(1)
     * 看了解析之后 发现一个 误区： minStack 中 不需要 保存 所有 入站元素 只需保存 比当前最小元素小的元素 就行 minStack 因此 就是有序的
